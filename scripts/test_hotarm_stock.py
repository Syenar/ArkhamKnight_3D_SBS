"""Hot-arm stock packer: boot stock+dxgi disarmed, F10-arm Present, verify SBS/AV."""
...
Start-Sleep 1
[N.K]::keybd_event(0x79,0,0,[UIntPtr]::Zero)  # F10
...
def main() -> None:
    ...
    if not alive or fatal:
        ...
        r = {"pass": False, "stage": "boot_disarmed", "max_mb": max_mb, "crash": crash, "fatal": fatal}
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("HOTARM_FAIL_BOOT", flush=True)
        return

    time.sleep(25)
    pre = analyze(capture("hotarm_pre.png"))
    print("PRE", pre, flush=True)

    # Arm packer + F10 reload
    subprocess.run(
    ...
    if not alive2 or fatal2:
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("HOTARM_FAIL_ARM", flush=True)
        return
    ...
    if not alive3:
        r = {"pass": False, "stage": "post_arm_watch", "crash": crash, "pre": pre, "max_mb": max_mb}
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("HOTARM_FAIL_WATCH", flush=True)
        return

    post = analyze(capture("hotarm_post.png"))
    ...
    if log.exists():
        ...
        hits = {
            ...
            "reload": t.lower().count("reload"),
        }

    result = {
        "pass": bool(post.get("pass")) and alive3,
        "alive": alive3,
        "max_mb": max_mb,
        "pre": pre,
        "post": post,
    ...
    print("HOTARM_SBS" if result["pass"] else "HOTARM_ALIVE_NO_SBS", flush=True)
