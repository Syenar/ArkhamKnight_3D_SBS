...
V13 = PROJ / "experimental_fork_20260724_nullguards" / "patched_dlls" / "d3d11.dll.patched_v14"
OUT = WC / "TEST_V14_SBS.txt"
SHOT = WC / "probe_shots"
...
def install(packer: str) -> None:
    ...
    assert sha(LIVE / "d3d11.dll") == "36B71A232FF4A641D20BFF6BC35E9FD08840E455ACFB0A6E0B3D5DE7B899C975"
...
def main() -> None:
    ...
    lines = [f"TEST_V14 {time.strftime('%Y-%m-%d %H:%M:%S')}"]
    for r in results:
        lines.append(str(r))
    ...
    if winner:
        (WC / "V14_SBS_WINNER.txt").write_text(str(winner) + "\n", encoding="utf-8")
        # lock recipe into STACK note
        (WC / "STACK_V14.txt").write_text(
            "v0.7.0-v14 — packer alive path (verify visually before claiming)\n"
            f"- d3d11 = patched_v14 ({sha(V13)[:16]}…)\n"
        ...
        print("LEFT_RUNNING_V14", winner["packer"], flush=True)
    else:
