"""v11 native call-through + minimal upscale with [Present] inside upscale.ini."""
...
TOOL = PROJ / "experimental_fork_20260724_nullguards" / "tools" / "patch_d3d11_v11_vs_ps.py"
V11 = PROJ / "experimental_fork_20260724_nullguards" / "patched_dlls" / "d3d11.dll.patched_v11"
OUT = WC / "TEST_V11_MINPRESENT.txt"
SHOT = WC / "probe_shots"
...
def main() -> None:
    subprocess.run(["python", str(TOOL), str(STOCK / "d3d11.dll"), str(V11)], check=True)
    subprocess.Popen(
        [
            "powershell",
            "-NoProfile",
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("V11MIN_FAIL_BOOT", flush=True)
        return
    ...
    if ps("(Get-Process BatmanAK -EA SilentlyContinue).Id").strip() == "":
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("V11MIN_FAIL_WATCH", flush=True)
        return

    a0 = analyze(capture("v11min_pre.png"))
...
"""
    ...
    time.sleep(2)
    shot = capture("v11min_post_t.png")
    a1 = analyze(shot)
    ...
    print("V11MIN_SBS" if result["pass"] else "V11MIN_ALIVE_NO_SBS", flush=True)
