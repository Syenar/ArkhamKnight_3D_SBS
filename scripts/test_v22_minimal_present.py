"""v22 + minimal CustomShaderUpscale with [Present] INSIDE upscale.ini (name resolves)."""
...
; No BuiltInCommandListUnbindAllRenderTargets — that path AVs at 0x1a8180 on AMD.
...
def main() -> None:
    ...
    (LIVE / "ShaderFixes" / "upscale.ini").write_text(MIN_UPSCALE, encoding="utf-8")
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("MINPRESENT_FAIL_BOOT", flush=True)
        return
    ...
    if ps("(Get-Process BatmanAK -EA SilentlyContinue).Id").strip() == "":
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("MINPRESENT_FAIL_WATCH", flush=True)
        return

    a0 = analyze(capture("v22min_pre.png"))
...
"""
    ...
    time.sleep(2)
    shot = capture("v22min_post_t.png")
    a1 = analyze(shot)
    ...
    if log.exists():
        ...
        for k in (
            "StereoProfile",
            "customshader\\shaderfixes\\upscale.ini\\upscale",
            "Unrecognised entry",
            "Operand type 50",
            "upscaling_switch",
            "Stereo disabled",
        ):
            hits[k] = t.lower().count(k.lower()) if k != "customshader\\shaderfixes\\upscale.ini\\upscale" else t.lower().count("customshader\\shaderfixes\\upscale.ini\\upscale")
        present_unrecog = t.lower().count("unrecognised entry: run = customshaderupscale")
    ...
    result = {
        ...
        "shot": str(shot),
        "hits": hits,
        "present_unrecog": present_unrecog,
        "d3d": sha(LIVE / "d3d11.dll")[:16],
    }
    OUT.write_text(str(result) + "\n", encoding="utf-8")
    print(result, flush=True)
    print("MINPRESENT_SBS" if result["pass"] else "MINPRESENT_ALIVE_NO_SBS", flush=True)
