"""v22 + stock upscale.ini [Present] (name resolves) + loader dxgi + mode1."""
...
OUT = WC / "TEST_V22_STOCK_UPSCALE.txt"
SHOT = WC / "probe_shots"
...
def analyze(path: Path) -> dict:
    ...
    # Title text only on left half is a classic true SBS cue on this screen
    title_left = float(left[:, : mid // 2].mean())  # noqa: placeholder
    return {
        "dual_fps": dual,
        "gL": gL,
        "gR": gR,
...
def main() -> None:
    ...
    # CRITICAL: stock upscale.ini keeps [Present] run = CustomShaderUpscale
    # (namespaced CustomShader only resolves from the include's Present)
    shutil.copy2(STOCK / "ShaderFixes" / "upscale.ini", LIVE / "ShaderFixes" / "upscale.ini")
    ...
    # Do NOT put run=CustomShaderUpscale in main Present — include handles it.
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("V22STOCK_FAIL_BOOT", flush=True)
        return
    ...
    if ps("(Get-Process BatmanAK -EA SilentlyContinue).Id").strip() == "":
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("V22STOCK_FAIL_WATCH", flush=True)
        return

    a0 = analyze(capture("v22stock_pre.png"))
...
"""
    ...
    time.sleep(2)
    a1 = analyze(capture("v22stock_post_t.png"))
    ...
    if log.exists():
        ...
        for k in (
            ...
            hits[k] = t.count(k)
        warn_unrecog = t.lower().count("unrecognised entry: run = customshaderupscale")
    ...
    result = {
        ...
        "final": a1,
        "hits": hits,
        "present_unrecog": warn_unrecog,
        "d3d": sha(LIVE / "d3d11.dll")[:16],
        "dxgi": sha(LIVE / "dxgi.dll")[:16],
    }
    OUT.write_text(str(result) + "\n", encoding="utf-8")
    print(result, flush=True)
    print("V22STOCK_SBS" if result["pass"] else "V22STOCK_ALIVE_NO_SBS", flush=True)
