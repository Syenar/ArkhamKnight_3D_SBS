"""v23 NullRdxSoft + native call-through + minimal upscale with [Present] inside upscale.ini."""
...
TOOL = PROJ / "experimental_fork_20260724_nullguards" / "tools" / "patch_d3d11_v23_nullrdx_soft.py"
V23 = PROJ / "experimental_fork_20260724_nullguards" / "patched_dlls" / "d3d11.dll.patched_v23"
OUT = WC / "TEST_V23_MINPRESENT.txt"
SHOT = WC / "probe_shots"
...
def main() -> None:
    subprocess.run(["python", str(TOOL), str(STOCK / "d3d11.dll"), str(V23)], check=True)
    subprocess.Popen(
        [
            "powershell",
            "-NoProfile",
    ...
    shutil.copy2(V23, LIVE / "d3d11.dll")
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("v23min_FAIL_BOOT", flush=True)
        return
    ...
    if ps("(Get-Process BatmanAK -EA SilentlyContinue).Id").strip() == "":
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("v23min_FAIL_WATCH", flush=True)
        return

    a0 = analyze(capture("v23min_pre.png"))
...
"""
    ...
    time.sleep(2)
    shot = capture("v23min_post_t.png")
    a1 = analyze(shot)
    ...
    if log.exists():
        ...
        present_unrecog = hits["Unrecognised"]
    ...
    print("v23min_SBS" if result["pass"] else "v23min_ALIVE_NO_SBS", flush=True)
