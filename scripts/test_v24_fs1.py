...
def main() -> None:
    ...
    ini = re.sub(r"(?m)^force_stereo\s*=\s*.*$", "force_stereo = 1", ini)
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("FS1_FAIL_BOOT", flush=True)
        return

    time.sleep(35)
    shot = capture("v24_fs1.png")
    a1 = analyze(shot)
    ...
    print("FS1_SBS" if result["pass"] else "FS1_ALIVE_NO_SBS", flush=True)
