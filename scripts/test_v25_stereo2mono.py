"""v25 + upscale Present using stereo2mono f_bb (explicit reverse blit)."""
...
TOOL = PROJ / "experimental_fork_20260724_nullguards" / "tools" / "patch_d3d11_v25_inject_loopguard.py"
DLL = PROJ / "experimental_fork_20260724_nullguards" / "patched_dlls" / "d3d11.dll.patched_v25"
OUT = WC / "TEST_V25_STEREO2MONO.txt"
SHOT = WC / "probe_shots"
...
def main() -> None:
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("V25S2M_FAIL_BOOT", flush=True)
        return

    time.sleep(35)
    shot = capture("v25_stereo2mono.png")
    a1 = analyze(shot)
    ...
    print("V25S2M_SBS" if result["pass"] else "V25S2M_ALIVE_NO_SBS", flush=True)
