"""v24 native-only no NullRdx + minimal upscale with [Present] inside upscale.ini."""
...
OUT = WC / "TEST_V24_MINPRESENT.txt"
SHOT = WC / "probe_shots"
...
def main() -> None:
    ...
    if not alive or fatal:
        ...
        print(r, flush=True)
        print("V24_FAIL_BOOT", flush=True)
        return
    ...
    if ps("(Get-Process BatmanAK -EA SilentlyContinue).Id").strip() == "":
        ...
        OUT.write_text(str(r) + "\n", encoding="utf-8")
        print(r, flush=True)
        print("V24_FAIL_WATCH", flush=True)
        return

    a0 = analyze(capture("v24_pre.png"))
...
"""
    ...
    print("V24_SBS" if result["pass"] else "V24_ALIVE_NO_SBS", flush=True)
